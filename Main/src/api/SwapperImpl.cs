...
public interface SwapperBase {
    ...
    static Type? getBaseType<S>() where S : SwapperBase {
        var type = typeof(S);

        if (typeof(MeshSwapper).IsAssignableFrom(type)) {
            return typeof(MeshSwapper);
        }
        else if (typeof(GeneralSwapper).IsAssignableFrom(type)) {
            return typeof(GeneralSwapper);
        }
...
public class AudioSwapper(Identifier id, AudioClip clip) : SwapperImpl(id), GeneralSwapper {
    ...
    public void onSwap(GameObject gameObject, List<Component> trackedComponents) {
        ...
        source.minDistance = Plugin.config.minAudioDistance();
        source.maxDistance = Plugin.config.maxAudioDistance();
