...
public static class MediaFormats {
    public static readonly Endec<MediaFormat> FORMAT_ENDEC = endec.Endec.STRING.xmap(s => MediaFormats.getFormat(s), type => type.name());
...
public class WEBM : MediaFormat {
    ...
    public static SwapperImpl? createHandler(Identifier id, RawMediaData data) {
        var hasCachedFile = data.attemptToCacheFile(bytes => {
            ...
            if (!Plugin.config.allowTranscodingVideos()) {
