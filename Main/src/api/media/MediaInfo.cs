...
public record MediaInfo : EndecGetter<MediaInfo> {
    ...
    public static readonly Endec<MediaInfo> ENDEC = StructEndecBuilder.of(
            endec.Endec.INT.fieldOf<MediaInfo>("width", s => s.width),
            endec.Endec.INT.fieldOf<MediaInfo>("height", s => s.height), 
            MediaFormats.FORMAT_ENDEC.optionalFieldOf<MediaInfo>("ext", s => s.format, new UnimplementedMediaFormat("unknown")),
            endec.Endec.INT.optionalFieldOf<MediaInfo>("size", s => s.size, () => 0), 
            endec.Endec.STRING.optionalFieldOf<MediaInfo>("md5", s => s.md5Hash, () => ""),
            endec.Endec.STRING.fieldOf<MediaInfo>("url", s => s.uri),
            endec.Endec.DOUBLE.optionalFieldOf<MediaInfo>("duration", s => s.duration, () => -1),
            endec.Endec.STRING.optionalFieldOf<MediaInfo>("video_codec", s => s.videoCodec, () => ""),
            endec.Endec.STRING.optionalFieldOf<MediaInfo>("audio_codec", s => s.audioCodec, () => ""),
            endec.Endec.INT.optionalFieldOf<MediaInfo>("channels", s => s.channels, () => -1),
            endec.Endec.INT.optionalFieldOf<MediaInfo>("frequency", s => s.frequency, () => -1),
            endec.Endec.BOOLEAN.optionalFieldOf<MediaInfo>("is_error", s => s.isError, () => false),
