...
public class MediaSwapperStorage {
    ...
    public static SwapperBase? getHandler(Identifier identifier) => ID_TO_SWAPPER.GetValueOrDefault(identifier);
    
    public static S? getHandler<S>(Identifier identifier) where S : SwapperBase {
        return typeof(MeshSwapper).IsAssignableFrom(typeof(S)) ? (S) ID_TO_SWAPPER.GetValueOrDefault(identifier) : default;
    }
    
    public static bool hasMaterial(Identifier identifier) => ALL_MEDIA_IDS.Contains(identifier);
