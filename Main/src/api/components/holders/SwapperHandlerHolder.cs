...
public class SwapperHandlerHolder : MonoBehaviour {
    ...
    public List<Identifier> getAppliedSwapIds() {
        List<Identifier> ids = new ();
        
        var generalId = _generalSwapper.swapper?.id();
        
        if (generalId != null) ids.Add(generalId);
        
        ids.AddRange(_meshToCurrentHandlers
                .Values
                .SelectMany(instances => instances.Values)
                .Select(instance => instance.swapper?.id())
                .selectNonNull());
